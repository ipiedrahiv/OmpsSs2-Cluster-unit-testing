...
bool test_flush_strong_accesses()
{
	...
	#pragma oss task in(buff[0;ARRAY_SZ]) out(sum[0;tasks_nr]) label("sum_decompose")
	...
	#pragma oss taskwait
	/* After this, 'buff' and 'sum' are in home node and success is here */
...
{
	test_t test = FUNC_DEF(test_flush_strong_accesses);
