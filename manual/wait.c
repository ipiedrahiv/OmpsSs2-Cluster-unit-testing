...
bool test_wait()
{
	...
	#pragma oss task out(buff[0;ARRAY_SZ]) label("task0")
	init_buff_range_int(buff, ARRAY_SZ, 1);

	#pragma oss task out(sum[0;tasks_nr]) label("task1")
	init_buff_value_int(sum, tasks_nr, 0);

	#pragma oss task in(buff[0;ARRAY_SZ]) inout(sum[0;tasks_nr]) label("sum_decompose") wait
...
{
	test_t test = FUNC_DEF(test_wait);
