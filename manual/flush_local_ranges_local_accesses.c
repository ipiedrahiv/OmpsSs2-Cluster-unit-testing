...
bool test_flush_local_ranges_local_accesses()
{
	...
	for (size_t i = 0; i < ARRAY_SZ; i += TASK_SZ) {
		#pragma oss task in(buff[i;TASK_SZ]) inout(sum[0;tasks_nr]) label("sum_task")
...
{
	test_t test = FUNC_DEF(test_flush_local_ranges_local_accesses);
